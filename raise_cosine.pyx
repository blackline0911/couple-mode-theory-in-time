...
cpdef raise_cos(cnp.ndarray[float_data_type, ndim=1] BS,t,shift,beta,T):
    ...
    if isinstance(t,cnp.ndarray):
        ans = np.where( ( (t-shift)==T/2/beta) | ( (t-shift)==( -T/2/beta) ),
                        BS[int(shift/T)]*np.pi/4*sinc((1/2/beta)),
                        BS[int(shift/T)] * sinc((t-shift)/T) * np.cos(np.pi*beta*(t-shift)/T) / (1-(2*beta*(t-shift)/T)**2) )
        return ans
    #else:
        if ( (t-shift)==T/2/beta) | ( (t-shift)==( -T/2/beta) ):
            s = BS[int(shift/T)]*np.pi/4*sinc((1/2/beta))
        else:
            s = BS[int(shift/T)]*sinc((t-shift)/T)*np.cos(np.pi*beta*(t-shift)/T)/(1-(2*beta*(t-shift)/T)**2)
    return s
cpdef create_rcos_signal(cnp.ndarray[float_data_type, ndim=1] BS,
    ...
    for i in range(N):
        ...
        #for j in range(N):
        rcos_signal += raise_cos(BS,t,(i)*T_normalized,beta,T_normalized)
    return rcos_signal