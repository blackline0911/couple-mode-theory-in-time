...
def solving(ring, 
    ...
    def CMT(t_bar,eqs):
        ...
        f1 = 1j*(ring.w_res_bar-w_pround_bar)*b_bar- (1/ring.tu_t_bar)*b_bar + sqrt(2/ring.tu_e_bar) *1 + 1j*D_bar*(-ring.me*1e-12/1e-6)*Q_pround*b_bar
...
#     """
    ...
    b_init=0+1j*0
    Q_init=0
    # notes: time_range argument should be slightly include the t_eval
    for i in range(time.N):
        if i==0:
            sol =  solve_ivp(CMT ,[0 ,time.t_all_segment[0][-1]], [b_init, Q_init],method=method,t_eval = time.t_all_segment[i] ,atol = atol,rtol = rtol)
            b = sol.y[0]
            b_record = np.append(b_record,sol.y[0])
            q = sol.y[1]
            Q_record = np.append(Q_record,sol.y[1])
        else:
            sol =  solve_ivp(CMT ,[time.t_all_segment[i-1][-1] ,time.t_all_segment[i][-1]], [b_init, Q_init],method=method,t_eval = np.append(  np.array([time.t_all_segment[i-1][-1]]), np.array(time.t_all_segment[i])  ),atol = atol,rtol = rtol)
            b = sol.y[0]
