...
sdp.simple_eye(signal_ideal[100*samples_per_symbol:], samples_per_symbol*3, 500, t_sample, res=dpi, title="ideal {}Gbps 4-PAM Signal".format(data_rate/1e9))
plt.show()
...
sdp.simple_eye(signal_ideal, samples_per_symbol*3, 500, 500*1e-15, "{}Gbps Ideal 4-PAM Signal".format(data_rate/1e9),res=100)
plt.show()
#%%
...
sdp.simple_eye(signal_jitter, samples_per_symbol*3, 500, 500*1e-15, "{}Gbps 4-PAM Signal with jitter".format(data_rate/1e9),linewidth=lw,res=100)
plt.show()
#signal at receiver with no jitter
signal_out_ideal = sp.signal.convolve(h,signal_ideal)
sdp.simple_eye(signal_out_ideal[100*samples_per_symbol:], samples_per_symbol, 5000, 500*1e-15, "rx eye diagram no jitter",linewidth=lw,res=100)
plt.show()
#signal at reciever with tx jitter
signal_out_jitter_tx = sp.signal.convolve(h,signal_jitter)
sdp.simple_eye(signal_out_jitter_tx[100*samples_per_symbol:], samples_per_symbol, 5000,  500*1e-15, "rx eye diagram with tx jitter",linewidth=lw,res=100)
plt.show()
#signal at receiver with rx jitter
sdp.rx_jitter_eye(signal_out_ideal[100*samples_per_symbol:],samples_per_symbol,5000,5000,500*1e-15,"rx eye diagram with rx_jitter",stdev=1000e-15,linewidth=lw,res=100)
plt.show()