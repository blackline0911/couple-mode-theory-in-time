...
if sim.mode == "scan_frequency":
    ...
    t.main(ring_mod,t_max=5000,resolution=2,buffer=550)
...
if sim.mode=="voltage_drive":
    ...
    ploting(wl*1000,10*np.log10(data_NL),10*np.log10(data),x_label='wavelength (nm)',title='Transfer function',filename='T',leg=['Pin = '+str(sim.Pin),'Pin = '+str(sim1.Pin)])
...
np.savetxt('s_output_power.txt',abs(s_minus)**2,fmt="%.8f", delimiter="\n")
